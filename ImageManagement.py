...
class loadImageThread(threading.Thread):
    ...
    def run(self):
        for image_url in self.image_list:
            ...
            self.mDowloaded_list.append(os.path.abspath(self.filepath+image_url))
def loadOnlineImgMul(website,image_list,thread_num,dowloaded_image_list,FilePath):
    ...
    for i in xrange(thread_num+1):
        if (i+1)*num_per_thread > len(image_list):
            t = loadImageThread(website,image_list[i*num_per_thread:-1],dowloaded_image_list,FilePath)
            t.start()
        else:
            t = loadImageThread(website,image_list[i*num_per_thread:(i+1)*num_per_thread],dowloaded_image_list,FilePath)
            t.start()
